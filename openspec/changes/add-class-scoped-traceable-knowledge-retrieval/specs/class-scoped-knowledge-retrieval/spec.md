...
When query embedding generation is available, the system SHALL retrieve class-scoped lexical candidates and class-scoped vector candidates, combine their ranks using a documented deterministic fusion rule, and return results ordered by the fused ranking.
...
- **THEN** the response identifies the retrieval mode as `hybrid` and ranks the union of eligible lexical and vector candidates
...
- **WHEN** a knowledge entry has no ready embedding for the configured embedding model
- **THEN** it is excluded from vector candidates but remains eligible for lexical retrieval
